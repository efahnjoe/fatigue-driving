...
async function buildClient() {
  try {
    ...
    const outDir = getOutDir(type, "linux", "client");

    await clean(outDir);
    await mkdir(outDir, { recursive: true });

    await copy(join(client_outdir, type, "linux"), outDir);
  } catch (error) {
...
async function buildServer() {
  try {
    ...
    const outDir = getOutDir(type, "linux", "server");

    await clean(outDir);
    await mkdir(outDir, { recursive: true });

    await copy(join(server_outdir, server_name), outDir);
  } catch (error) {
